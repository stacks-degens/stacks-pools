...
export function getEnv(key: string) {
  const value = process.env[key] || process.env[key.replace(/^SUPPLIER/, 'OPERATOR')];
...
export function getStxAddressSwapper(stxPrivateKeySwapper: string) {
  const networkVersion = getStxNetworkVersion();
  return makeStxAddress(stxPrivateKeySwapper, networkVersion);
}
...
export function validateConfig() {
  ...
  console.log(Uint8Array.from(Buffer.from('00', 'hex')));
  console.log(getPublicKey());
  console.log('BYTE ARRAY:', Uint8Array.from(getPublicKey()));
  return {
    ...keys,
