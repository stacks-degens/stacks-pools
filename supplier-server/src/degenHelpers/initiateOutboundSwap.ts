...
import { stacksProvider, bridgeContract } from '../stacks';
// import { bpsToPercent, btcToSats, satsToBtc, shiftInt, stxToUstx } from '../utils';
import {
  getBtcAddress,
  getNetworkKey,
  getPublicKey,
...
import { askStxFee, broadcastAndLog, broadcastAndLogSwapper } from '../../scripts/helpers';
import { PostConditionMode } from 'micro-stacks/transactions';
...
async function run() {
  ...
  const initiateOutboundSwapTx = bridge.initiateOutboundSwap(
    BigInt(txAmount),
    Uint8Array.from(Buffer.from('00')),
    Uint8Array.from(btcPublicKey),
    BigInt(1)
  );
