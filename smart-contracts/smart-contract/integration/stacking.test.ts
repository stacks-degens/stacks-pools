...
// HOW TO RUN:
// npm run test:dev ./integration/stacking.test.ts
describe(
        ...
        if (txs.length > 1) {
          for (let i = 1; i <= txs.length - 1; i++) {
            ...
            if (txMethod == 'allow-contract-caller') {
              expect(txSC as any).toBe(`${Contracts.POX_4.address}.${Contracts.POX_4.name}`);
