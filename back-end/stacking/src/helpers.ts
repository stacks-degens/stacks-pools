...
export const getEvents = async () => {
  ...
  for (const entry of parsedEvents) {
    if (
      ...
      entry?.contract_log?.value?.repr?.includes(POOL_CONTRACT_ADDRESS)
    ) {
...
export const parseEvents = async (events: any) => {
...
export const getRewardIndexesMap = async (currentCycle: number) => {
  ...
  while (continueFetching) {
    ...
    if (rewardCycleIndexData.value === null) {
      if (rewardCycle > currentCycle + 1 && rewardIndex === 0) {
...
const processTransactions = async (
  ...
  const rewardAddress = await REWARD_BTC_ADDRESS();
      ...
      switch (transaction.functionName) {
        ...
        case 'delegate-stack-extend':
          if (transaction.poxAddress === rewardAddress) {
            ...
            console.log(
              `Delegation from ${transaction.stacker} could not be extended, as the reward address doesn't match with the one in the contributor contract.`
            );
          }
          break;
...
export const checkAvailableTransactions = (
  ...
  delegations.forEach(async (value: any, key: any) => {
    if (!acceptedDelegations.has(key)) {
      const maxCycles = Math.min(
        value.endCycle !== null ? value.endCycle - currentCycle - 1 : 1,
        1
      );
      const operation = {
  ...
  poxAddressSet.forEach(async (address) => {
    ...
    const maxEndCycle = Math.min(currentCycle + 2, maxEndCycleList);
    ...
    delegations.forEach(
      async ({ startCycle, endCycle, amountUstx, rewardIndex }: any) => {
        for (
          ...
          cycle < (endCycle !== null ? endCycle : currentCycle + 2);
          cycle++
        ) {
          const finalAmount = finalCycles.get(cycle) || 0;
          if (amountUstx < finalAmount && startCycle <= currentCycle + 1) {
            const operation = {
